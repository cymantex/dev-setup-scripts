...
1. \`git clone https://github.com/dv16sen/${process.argv[3]}.git\`
...
const getPackageJson = () => {
    const packageClone = JSON.parse(JSON.stringify(packageJson));
    packageClone.name = process.argv[3];
    packageClone.scripts = {
        ...
        "updateRoutes": "node client/scripts.js updateRoutes",
        "removeDatabase": "ts-node scripts.ts removeDatabase",
        "defaultDatabase": "ts-node scripts.ts defaultDatabase",
        "heroku-postbuild": "npm run init && npm run build"
    };
    return JSON.stringify(packageClone, null, 2);
};
...
export default async (): Promise<{}> => {
    ...
    return new Promise(resolve => {
        fs.writeFile(`${root}/package.json`, getPackageJson(), (err) => {
            if(err) throw err;
            console.log("package.json updated!");
            resolve();
        })
    }).then(() => new Promise(resolve => {
        fs.writeFile(`${root}/README.md`, getReadme(), (err) => {
            if(err) throw err;
